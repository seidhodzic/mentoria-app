-- =============================================================================
-- Mentoria MVP — FULL SUPABASE CATCH-UP (run in SQL Editor on a stale project)
-- =============================================================================
-- Use when your remote DB is behind the app (missing columns, tables, RLS, or
-- storage policies). Matches repo root schema.sql.
--
-- Empty DB: safe full bootstrap. Existing DB: uses IF NOT EXISTS and DROP/CREATE
-- policies; backup production first and fix any conflicts with custom objects.
--
-- After success: regenerate types (Dashboard → API, or supabase gen types).
-- =============================================================================
...
-- Mentor/admin check for profiles RLS — subquerying profiles inside a profiles policy causes
-- "infinite recursion detected in policy for relation profiles". Use SECURITY DEFINER.
...
begin
  ...
  values (
    ...
    case
      when coalesce(nullif(trim(new.raw_user_meta_data ->> 'signup_access_type'), ''), '') = 'subscription' then 'pending'
      else 'active'
    end,
    ...
    case
      when coalesce(nullif(trim(new.raw_user_meta_data ->> 'signup_access_type'), ''), '') = 'subscription' then false
      else true
    end,
    null
  )
